...
public class ChimneyGroup {
	...
	private int topChimneyY = -360;
	private int bottomChimneyY = 170;
	...
	public ChimneyGroup() {
	try {
		chimneyImage = ImageIO.read(new File("Asserts/chimney.png"));
		chimneyImage2 = ImageIO.read(new File("Asserts/chimney_.png"));
    } catch (IOException e) {}
		...
		for (int i = 0; i < SIZE/2; i++) {
			...
			cn = new Chimney(830+i*300, topChimneyY + deltaY, 74, 400 + deltaY);
			chimneys.push(cn);
		}
	}
	...
	public void resetChimneys() {
		...
		for (int i = 0; i < SIZE/2; i++) {
			...
			int deltaY = getRandomY();
			cn = new Chimney(830+i*300, bottomChimneyY + deltaY, 74, 400 + deltaY);
			chimneys.push(cn);
